      ...
      <div className="grid md:grid-cols-2 gap-12 items-stretch">
        ...
        <div className="relative rounded-main overflow-hidden bg-neutral-900 text-white p-8 flex flex-col justify-between">
          ...
          <div className="relative z-10 space-y-5">
            <p className="text-sm uppercase tracking-widest text-white/70">{data.eyebrow}</p>
        ...
        >
          <div className="space-y-6">
            {data.fields.map((field, i) => {
              ...
              return (
                <div key={i} className="space-y-2">
                  ...
                  <div className="flex items-start gap-3 border border-neutral-200 rounded-main px-4 py-3 focus-within:ring-2 focus-within:ring-mainColor transition">
                    ...
                    {isTextarea ? (
                      <textarea
                        rows={4}
                        suppressHydrationWarning
                        placeholder={placeholders[field] || `Enter ${field.toLowerCase()}`}
                  ...
                  {helper[field] && (
                    <p className="text-xs text-neutral-400">{helper[field]}</p>
                  )}
                </div>
              );
            })}
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-main bg-mainColor text-white font-medium hover:opacity-90 transition-all flex items-center justify-center gap-2"
            >
