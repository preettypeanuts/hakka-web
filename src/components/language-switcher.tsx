...
export default function LanguageSwitcher() {
    ...
    return (
        ...
        >
            {/* Trigger */}
            <button
                className={`flex items-center gap-2 border bg-lightColor md:bg-transparent border-white/30 px-3 py-1.5 rounded-main backdrop-blur-sm transition-opacity duration-200 ${isPending ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}
            >
                <CurrentFlag />
                <span className="text-xs font-semibold tracking-widest uppercase text-darkColor md:text-white">
                    {current.code}
                </span>
                <svg
                    className={`w-3 h-3 text-secondaryDark/70 md:text-white/70 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
